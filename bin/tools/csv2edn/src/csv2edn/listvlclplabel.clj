...
(defn listvlclplabel []
  ...
  (layout/common 
   [:h3 "PDGM Value-Cluster-Label List"]
   ...
   (form-to [:post "/listvlclplabel-gen"]
            [:table
             ...
             [:tr [:td ]
              [:td [:input#submit
                    {:value "Make PDGM Value-Cluster-Label List", :name "submit", :type "submit"}]]]]))))
   ...
   [:body
    ;;[:h3#clickable "Value-clusters used in " pos " pdgms for: " ldomain]
      (let [lprefmap (read-string (slurp "pvlists/lprefs.clj"))
            langs (split ldomain #",")
            posvec (vector "pro" "nfv" "noun" "fv")]
        (for [language langs]
          (let [lang (read-string (str ":" language))
                lpref (lang lprefmap)
                outfile (str "pvlists/dataID-pdgm-" language ".edn")
                datamap (atom {})]
            (for [pos posvec]
              (let [query-sparql1 (cond 
                ...
                [:div [:h4 "======= Debug Info: ======="]
                 ...
                 [:pre req4-out]
                 [:p "==========================="]]
                ;; if parens like this, gives only last datamap
                ;; with parens shifted one level, give nothing
                (spit outfile @datamap))))))

...
==================================[03/19/19]
	...
	PREFIX aamag:	 <http://oi.uchicago.edu/aama/2013/graph/>
	PREFIX bva:   <http://id.oi.uchicago.edu/aama/2013/beja-van/> 
	SELECT ?number ?person ?gender ?tokenprefix ?tokenstem ?tokensuffix 
	WHERE
        { 
	 { 
	  GRAPH aamag:beja-van  
	   ...
	   ?s aamas:lang / rdfs:label ?langLabel .
           ?s ?QmorphClass bva:Finite. 
           ?s bva:conjClass  bva:Prefix .  
          ?s bva:polarity  bva:Affirmative .  
          ?s bva:rootClass  bva:CCC .  
          ?s bva:tam  bva:Aorist .  
          ?s bva:number / rdfs:label  ?number .
          ?s bva:person / rdfs:label  ?person .
          ?s bva:gender / rdfs:label  ?gender .
          ?s bva:token-prefix  ?tokenprefix .
          ?s bva:token-stem  ?tokenstem .
          ?s bva:token-suffix  ?tokensuffix .
	      } 
	     } 
            } 
            ORDER BY DESC(?number) ?person DESC(?gender) ?tokenprefix ?tokenstem ?tokensuffix 
	...
	PREFIX aamag:	 <http://oi.uchicago.edu/aama/2013/graph/>
	PREFIX bva:   <http://id.oi.uchicago.edu/aama/2013/beja-van/> 
	SELECT ?number ?person ?gender ?tokenmerge 
	WHERE
        { 
	 { 
	  GRAPH aamag:beja-van  
	   ...
	   ?s aamas:lang / rdfs:label ?langLabel .
          ...
          ?s bva:token-stem  ?tokenstem .
          ?s bva:token-suffix  ?tokensuffix .
    BIND (CONCAT (?tokenprefix, " # ", ?tokenstem, " # ", ?tokensuffix) AS ?tokenmerge)
	      } 
	     } 
            } 
            ORDER BY DESC(?number) ?person DESC(?gender) ?tokenmerge



(use '[clojure.string :only (join split)])
(def valstring "number,person,gender,token-prefix,token-stem,token-suffix")
(def selection2 "?number ?person ?gender ?tokenprefix ?tokenstem ?tokensuffix")
(def tokensel (str "?token" (last (split selection2 #" ?token" 2))))
(def tokenselvec (split tokensel #" "))
(def mergetokenstr (join ", \" #  \", " tokenselvec))
